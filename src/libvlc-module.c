...
#define AUDIO_DRIFT_SLEW_TEXT N_( \
    "Drift correction slew" )
#define AUDIO_DRIFT_SLEW_LONGTEXT N_( \
    "How long the detune takes to follow what the drift correction asks for, " \
    "in seconds. The delay reported by an audio device is noisy, and applying " \
    "that noise as detune wobbles the pitch instead of holding an offset. " \
    "Lower it to correct sooner, at the cost of following the noise." )
