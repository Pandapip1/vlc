...
struct aout_sys_t
{
    ...
    /* Audio held back while the device latency is measured: it cannot be read
     * until the stream has run, so the stream runs on silence until then. */
    struct {
        block_t *first;
        block_t **last;
    } fifo;
 ...
 *
 * Only the device is in the way; what PulseAudio reports counts the queue in
 * and leaves the device out while corked, so it is wrong in both directions.
 */
 ...
 * runs, then clamps the position at zero, so what PulseAudio reports is short
 * by the device latency until the stream has played as much as the device
 * holds - milliseconds on a local sink, over two hundred on A2DP.
 */
 ...
 *
 * A positive wait leaves a hole, played as silence. A negative one means
 * playback is already due, so seek on the read index instead: the audio goes
 * in over the silence written to get here rather than behind it.
 */
 ...
 *
 * The device latency is kept: a flush does not move the device, and it is what
 * lets the next start be timed without running on silence again.
 */
...
static void stream_start(pa_stream *s, audio_output_t *aout)
{
    ...
    /* prebuf is zero so the start is timed here rather than by the server,
     * which also means it starts on whatever is queued. After a flush the
 ...
 *
 * It cannot be topped up from Play(): holding the audio back fills the core's
 * FIFO, which stalls the decoder, which stops Play() being called at all. Let
 * the server ask for what it needs instead.
 */
...
static int Start(audio_output_t *aout, audio_sample_format_t *restrict fmt)
{
    ...
    /* Settle the clock before playback rather than during it: measuring once
     * audio is flowing leaves that audio late by the measurement, which the
     * core makes up by dropping a chunk of it. Nothing is playing yet, so the
     * wait only adds to start-up latency. */
    if (sys->device_latency == VLC_TICK_INVALID)
    {
        stream_silence(s, aout, AOUT_MIN_PREPARE_TIME);
        stream_start_now(s, aout);
