...
#define H26X_PROBE_LENGTH (2 * CLOCK_FREQ)
...
static int Demux( demux_t *p_demux)
{
                                                             ...
                                                             p_block_in ? &p_block_in : NULL )) )
    {
        while( p_block_out )
        {
            ...
            if( !p_sys->b_seeked && pcr > VLC_TICK_0 )
    ...
    if( b_probing &&
        ( b_eof || date_Get( &p_sys->output_dts ) - VLC_TICK_0 >= H26X_PROBE_LENGTH ) )
        StartSeek( p_demux );
 ...
 * to, so that the first access unit after the seek is dated from there.
 *****************************************************************************/
