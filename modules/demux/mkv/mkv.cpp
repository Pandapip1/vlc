...
static int Control( demux_t *p_demux, int i_query, va_list args )
{
    ...
    switch( i_query )
    {
        ...
        case DEMUX_GET_POSITION:
            ...
            /* A live or truncated stream has no duration to divide by, but the
             * caller is owed an answer all the same: callers other than the
             * input do not pre-initialise what they pass in. */
            if ( p_sys->f_duration > 0.0 )
